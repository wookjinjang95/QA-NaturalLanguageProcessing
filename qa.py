...
def stemming_the_question(words):
    ...
    #print(final)
    #print('##########')
    result = [(x,y) for (x,y) in final]
    for (x,y) in result:
        syns = wn.synsets(x)
        key = x+"."+y
        #print('########key########')
        #print(key)
        updated_syn = [x for x in syns if key in x.name()]
        #print('#######updated#######')
        #print(updated_syn)
        if y == 'n':
            for item in updated_syn:
                #print(item, item.lemma_names())
                for relative in item.lemma_names():
                    if relative != x:
                        final.append((relative, 'n'))
            all_synsets = mw.look_for_everything(syns)
            more = mw.look_for_noun_in_dataset(all_synsets)
            if more:
                for m in more:
                    final.append((m,'n'))
        ...
        if y == 'v':
            for item in updated_syn:
                ...
                for relative in item.lemma_names():
                    if relative != x:
                        final.append((relative, 'v'))
            all_synsets = mw.look_for_everything(syns)
            more = mw.look_for_verb_in_dataset(all_synsets)
            if more:
                for m in more:
                    final.append((m,'v'))
